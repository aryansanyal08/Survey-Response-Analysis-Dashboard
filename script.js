...
let currentViewType = "bar";

// Age groups we always combine
const AGE_GROUPS = ["18-23", "24-29", "30-35", "more-than-35"];
...
// Build datasets for BOTH chart types
function getChartDatasets(questionNum, isBar) {
    const datasets = [];

    AGE_GROUPS.forEach(age => {
        const q = surveyData[age].find(x => x.question === questionNum);
        if (!q) return;

        const total = Object.values(q.responses).reduce((s, v) => s + v, 0);

        const perc = [
            ((q.responses.i / total) * 100).toFixed(2),
            ((q.responses.ii / total) * 100).toFixed(2),
            ((q.responses.iii / total) * 100).toFixed(2),
            ((q.responses.iv / total) * 100).toFixed(2),
            ((q.responses.v / total) * 100).toFixed(2)
        ];

        const colors = {
            "18-23": "rgba(102,126,234,1)",
            "24-29": "rgba(118,75,162,1)",
            "30-35": "rgba(237,100,166,1)",
            "more-than-35": "rgba(255,154,158,1)"
        };

        datasets.push({
            label: age,
            data: perc,
            backgroundColor: isBar ? colors[age].replace("1)", "0.6)") : colors[age],
...
function updateQuestionText() {
    ...
    const text = questionTexts[currentQuestion] || "";
    box.innerHTML = `<strong>Q${currentQuestion}:</strong> ${text}`;
}
...
function updateTable() {
    ...
    AGE_GROUPS.forEach(age => {
        const q = surveyData[age].find(x => x.question === currentQuestion);
        if (!q) return;
        ...
        opts.forEach(opt => {
            ...
            row.innerHTML = `
                <td>${age}</td>
                <td>Option ${opt}</td>
                <td>${q.responses[opt]}</td>
                <td>${((q.responses[opt] / total) * 100).toFixed(2)}%</td>
            `;
        });
    });
}
...
function updateStatistics() {
    ...
    AGE_GROUPS.forEach(age => {
        const q = surveyData[age].find(x => x.question === currentQuestion);
        if (q) total += Object.values(q.responses).reduce((s, v) => s + v, 0);
    });
...
document.getElementById("downloadChart").addEventListener("click", () => {
    ...
    link.download = `Question_${currentQuestion}_chart.png`;
